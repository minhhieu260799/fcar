...
Ứng dụng website bán xe ô tô: danh sách xe, xem chi tiết xe, đặt cọc, đăng ký lái thử, liên hệ tư vấn, danh sách yêu thích, so sánh xe, đánh giá xe (có duyệt nội dung), và khu quản trị (admin) cho chi nhánh, hãng, dòng xe, phiên bản, mẫu xe, nhập kho, đơn hàng, đăng ký lái thử, liên hệ tư vấn, duyệt đánh giá, thanh toán, báo cáo - thống kê.
...
- **Admin:** sau khi đăng nhập tài khoản admin, vào **Admin** → sidebar với các mục:
  - Quản lý chi nhánh, hãng, dòng xe, phiên bản (thêm/sửa/disable/enable, form thêm trong modal căn giữa).
  - Quản lý mẫu xe: thêm mẫu xe mới, nhập xe mới vào kho, lịch sử nhập.
...
- **Logic dùng chung:** `StorefrontCarListingModelHelper.populateCarListing` gọi `CarQueryService.listDefinitionsForStorefront` để lấy **các mẫu xe (`CarDefinition`) đang được coi là niêm yết** (kho không tắt, mẫu và cây hãng / dòng / phiên bản hoạt động — khớp điều kiện trong service).
...
| Tham số | Ý nghĩa |
|---------|---------|
| `q` | Từ khóa tìm theo tên **hãng**, **dòng xe**, **phiên bản** (nếu có), và **năm sản xuất** (so khớp dạng chuỗi). Kết hợp với các lọc khác bằng **AND**. |
| `brands` | Danh sách ID hãng, định dạng **`id1,id2,...`**. |
| `models` | Danh sách ID dòng xe, **`id1,id2,...`**. |
| `segments` | Danh sách ID phiên bản, **`id1,id2,...`**. |
...
- **Tùy chọn checkbox** (hãng / dòng / phiên bản) được build từ các mẫu đang niêm yết: `CarQueryService.loadStorefrontFilterOptions()` + `CarDefinitionRepository.findDistinctListedBrandIds` (và tương tự cho model/segment).
...
- **Cascade UI:** khi chọn hãng, danh sách **dòng xe** chỉ hiện các dòng thuộc hãng đã chọn (ẩn bằng class, theo `data-brand-id`); tương tự **phiên bản** theo **dòng** (`data-model-id`). Nút **Xóa bộ lọc** xóa `q`, bỏ tick tất cả và gọi lại fragment.
...
- **Hiển thị:** Ảnh đại diện, carousel ảnh phụ (nếu có), **tiêu đề** (formatter hãng + dòng + … + năm), **nút tim** góc phải cạnh tiêu đề (thêm / bỏ yêu thích qua API, không rời trang), thông số cơ bản, accordion **Thông số khác** (thuộc tính tùy chỉnh), khối **Giá bán** + badge tồn + **tổng tồn kho toàn hệ thống**.
...
### Thanh toán đặt cọc (PayOS-only)

**Code chính:** `OrderController`, `PayOsDepositService`, `CarOrderDepositService`; admin: `AdminPaymentController`; entity `PaymentConfig`, `SupportHotline`.
...
- **Trang đặt cọc (`/orders/deposit/{definitionId}`):** Chỉ hỗ trợ PayOS. Nếu PayOS chưa cấu hình, hệ thống chặn luồng đặt cọc và redirect về trang chi tiết xe kèm thông báo lỗi.
- **PayOS (bật trong `application.yml` và đủ credential):** Nút **Thanh toán đặt cọc qua PayOS** → `POST /orders/deposit/payos/start` tạo phiên thanh toán và redirect sang PayOS; sau khi thanh toán thành công, **webhook / luồng return** (`/orders/deposit/payos/return`) hoàn tất tạo đơn **DEPOSITED** và trừ tồn. Cần có **cấu hình thanh toán** (repository `PaymentConfig`) — nếu chưa có bản ghi, luồng đặt cọc báo lỗi cấu hình.
...
- **Danh sách:** Hãng chưa xóa: ID, tên, trạng thái (Hoạt động / Tạm ngưng), thao tác.
- **Thêm:** Modal — tên; không trùng tên (so khớp không phân biệt hoa thường, trim). Có xác nhận trước khi gửi.
- **Cập nhật:** Chỉ đổi tên; không trùng tên khác. **Không cho đổi tên** nếu hãng đang được xe kho hoạt động sử dụng (theo brand trên `CarInventory`).
- **Tạm ngưng / Hoạt động (cascade — `CatalogActiveCascadeService`):**
  - **Tạm ngưng:** Không chặn theo số lượng tồn kho. Tắt `active` cho hãng, toàn bộ dòng xe, phiên bản, mẫu xe thuộc cây; đồng thời đặt `CarInventory.disabled = true` cho các dòng kho liên quan.
  - **Bật lại:** Bật hãng rồi cascade bật xuống dưới và mở lại `disabled = false` cho tồn kho tương ứng.
...
- **Danh sách & lọc:** Bảng dòng xe chưa xóa; có lọc theo hãng (tự submit form khi đổi dropdown).
- **Thêm:** Modal — chọn hãng, tên dòng; không trùng tên trong cùng hãng. Nếu hãng đang tạm ngưng, dòng xe mới mặc định không hoạt động.
- **Cập nhật:** Trang chỉnh sửa (theo route controller); không đổi hãng; không trùng tên. **Không cho sửa tên** khi dòng xe đang được xe kho hoạt động sử dụng.
- **Tạm ngưng / Hoạt động (cascade):**
  - **Tạm ngưng:** Cascade xuống phiên bản, mẫu xe và tồn kho (giống luồng hãng), không chặn theo tồn kho.
  - **Bật lại:** Chỉ khi **hãng cha** đang hoạt động; nếu không → toast lỗi (nêu rõ hãng đang tạm ngưng). Cascade bật xuống và mở tồn kho tương ứng.
- **Xóa:** Chỉ **xóa cứng** khi **không** còn **phiên bản** hoặc **mẫu xe** con trực tiếp (`deleted = false`). Nếu còn → toast lỗi, không xóa.

---

### 4. Quản lý phiên bản xe

**Đường dẫn:** `/admin/segments`

- **Danh sách & lọc:** Phiên bản chưa xóa; lọc theo hãng và dòng xe (form tự submit).
- **Thêm:** Modal — hãng, dòng xe, tên phiên bản; không trùng tên trong cùng dòng xe. Nếu dòng xe tạm ngưng, phiên bản mới mặc định không hoạt động.
- **Cập nhật:** Trang chỉnh sửa; chỉ đổi tên phiên bản khi hợp lệ.
- **Tạm ngưng / Hoạt động (cascade):**
  - **Tạm ngưng:** Tắt phiên bản, các mẫu xe thuộc phiên bản và tồn kho liên quan; không chặn theo tồn kho.
  - **Bật lại:** Chỉ khi **dòng xe cha** (con trực tiếp) đang hoạt động; nếu không → toast lỗi (nêu rõ dòng xe). Không còn kiểm tra riêng hãng trong message; cascade bật mẫu xe và tồn kho.
- **Xóa:** Chỉ **xóa cứng** khi **không** còn **mẫu xe** con trực tiếp (`CarDefinition` với `deleted = false` gắn phiên bản). Nếu còn → toast lỗi, không xóa.

---

### 5. Quản lý mẫu xe (mẫu xe + nhập xe)
...
- **Danh sách:** Các mẫu xe chưa xóa; có lọc theo hãng / dòng xe. Cột **Mẫu xe** hiển thị chuẩn: *Hãng + Dòng + Phiên bản - Năm* (qua `CarDefinitionLabelFormatter`).
- **Thêm mẫu xe:** Form (modal) — hãng, dòng, phiên bản, năm, kiểu dáng, giá, màu, ảnh, v.v.; không trùng bộ (hãng + dòng + phiên bản + năm) với mẫu đã có.
...
- **Tạm ngưng / Hoạt động (cascade tồn kho):**
  ...
  - **Bật lại:** Chỉ khi **phiên bản** cha còn hoạt động (hoặc nếu không có phiên bản thì kiểm tra **dòng xe**); nếu không → toast lỗi. Khi bật: `active = true` và mở lại `disabled = false` cho tồn kho tương ứng.
...
**Ảnh hưởng storefront:** Trang chủ và danh sách xe chỉ hiển thị xe thỏa điều kiện (kho không `disabled`, mẫu và cây hãng/dòng/phiên bản đang hoạt động, v.v.). Phiên bản `null` vẫn được xử lý trong truy vấn tìm kiếm. Chi tiết xe (`/cars/{id}`) trả **404** nếu xe không còn được coi là “niêm yết”.
