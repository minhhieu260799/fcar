...
/** Phân công: Hiếu — quản lý phiên bản xe (admin). */
@Controller
...
public class AdminSegmentController {
    ...
    public String list(
            ...
            Model model) {
        // Bộ lọc: hiển thị cả hãng/dòng xe đang tạm ngưng (trừ khi đã xóa)
    ...
    public String create(@Valid @ModelAttribute("form") SegmentForm form,
        ...
        if (carModel != null && segmentRepository.existsByModelAndName(carModel, form.getName())) {
            bindingResult.rejectValue("name", "name.exists", "Phiên bản đã tồn tại trong dòng xe này");
        }
        ...
        // Nếu dòng xe đang tạm ngưng, phiên bản mới cũng mặc định tạm ngưng
        segment.setActive(carModel.isActive());
        segmentRepository.save(segment);
        redirectAttributes.addFlashAttribute("success", "Thêm phiên bản thành công.");
    ...
    public String disable(@PathVariable Long id, RedirectAttributes redirectAttributes) {
        ...
        redirectAttributes.addFlashAttribute("success", "Đã tạm ngưng phiên bản và các mục liên quan.");
        return "redirect:/admin/segments";
    }
    ...
    public String enable(@PathVariable Long id, Model model, RedirectAttributes redirectAttributes) {
        ...
        redirectAttributes.addFlashAttribute("success", "Đã bật hoạt động phiên bản và các mục liên quan.");
        return "redirect:/admin/segments";
    }
    ...
    public String delete(@PathVariable Long id, RedirectAttributes redirectAttributes) {
        ...
        if (carDefinitionRepository.existsBySegmentAndDeletedFalse(segment)) {
            redirectAttributes.addFlashAttribute("error",
                    "Phiên bản không thể xóa khi đã tồn tại mẫu xe trực tiếp.");
            return "redirect:/admin/segments";
        }
        segmentRepository.delete(segment);
        redirectAttributes.addFlashAttribute("success", "Đã xóa phiên bản.");
        return "redirect:/admin/segments";
    }
